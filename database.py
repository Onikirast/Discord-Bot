...
Schema design notes:
- All timestamps are stored as ISO-8601 strings in UTC. Conversion to a
  user's local time happens only at display time.
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            ...
            (user_id, channel_id, message, remind_at.isoformat(),
             recurrence, datetime.utcnow().isoformat()),
        )
        await db.commit()
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            ...
            (user_id, channel_id, day_of_week, start_time, title,
             notify_before_minutes, datetime.utcnow().isoformat()),
        )
        await db.commit()
        return cursor.lastrowid
...
async def set_dcu_link(user_id: int, course_name: str, course_description: str | None,
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            ...
            (user_id, course_name, course_description, course_identity,
...
async def add_grade(user_id: int, module_name: str, grade: float,
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?)",
            (user_id, module_name, grade, credits, semester, datetime.utcnow().isoformat()),
        )
        await db.commit()
        return cursor.lastrowid
