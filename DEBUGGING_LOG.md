...
## What this shows, for CV purposes

This debugging sequence is a good example of:
- **Systematic diagnosis** — reading full tracebacks rather than guessing, isolating environment issues from code issues
- **Defensive engineering** — the cog-wide error handler (bug #4) was added specifically so *future* unknown failures fail safely rather than silently
- **Adapting when an assumption breaks** — bug #6 wasn't fixed by patching around a symptom; it was root-caused by reading a second, more reliable source and rebuilding the integration on firmer ground
- **Verifying fixes, not just applying them** — several fixes above were confirmed with direct tests (simulated timeouts, datetime edge cases, cog-loading checks) rather than assumed to work

**Bottom line:** every feature in this bot — reminders (one-off and recurring), the manual timetable, and the live DCU integration — has now been exercised against either a real Discord server or real DCU data, not just code-reviewed. That's a meaningfully stronger claim to be able to make about a CV project than "it should work."
